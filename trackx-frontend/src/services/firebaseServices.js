...
const EVIDENCE_COLLECTION = 'evidence'; // Evidence collection for cross-case evidence
...
/**
* Save evidence item to global evidence collection
* @param {Object} evidenceItem - Evidence item to save
* @param {string} userId - User ID who created the evidence
* @param {string} caseNumber - Case number this evidence belongs to
* @returns {Promise<string>} - Returns the evidence ID
*/
...
try {
  ...
  if (existingDoc.exists()) {
    ...
    const relatedCases = existingData.relatedCases || [];
    
    // Add current case number if not already included
    if (caseNumber && !relatedCases.includes(caseNumber)) {
...
/**
* Load all evidence items for a specific case
* @param {string} caseNumber - Case number to filter by
...
try {
  ...
  const q = query(
    evidenceRef,
    where("relatedCases", "array-contains", caseNumber)
  );
  ...
  querySnapshot.forEach((doc) => {
    const data = doc.data();
    evidenceItems.push({
      id: data.evidenceId || data.id || doc.id,
      evidenceId: data.evidenceId || data.id || doc.id,
      description: data.description || "",
      caseNumber: data.originalCaseNumber || caseNumber,
      dateAdded: data.dateAdded || data.createdAt?.toDate?.()?.toISOString() || new Date().toISOString(),
      relatedCases: data.relatedCases || [],
    });
  });
  
  console.log(`Loaded ${evidenceItems.length} evidence items for case ${caseNumber}`);
...
/**
* Load all evidence items (for searching/reusing existing evidence)
...
try {
  ...
  console.log(`Loaded ${evidenceItems.length} evidence items`);
  return evidenceItems;
} catch (error) {
...
/**
* Search evidence by description (for reusing existing evidence)
* @param {string} searchTerm - Search term to filter by
* @returns {Promise<Array>} - Array of matching evidence items
*/
...
try {
  ...
  querySnapshot.forEach((doc) => {
    ...
    if (
      ...
      data.originalCaseNumber?.toLowerCase().includes(lowerSearch) ||
      data.evidenceId?.toLowerCase().includes(lowerSearch)
    ) {
      evidenceItems.push({
        id: data.evidenceId || data.id || doc.id,
        evidenceId: data.evidenceId || data.id || doc.id,
  ...
  console.log(`Found ${evidenceItems.length} evidence items matching "${searchTerm}"`);
  return evidenceItems;
} catch (error) {
...
* @param {string} evidenceId - Evidence ID to update
...
try {
  ...
  console.log('Evidence updated successfully:', evidenceId);
} catch (error) {
...
/**
* Batch save multiple evidence items
* @param {Array} evidenceItems - Array of evidence items to save
* @param {string} userId - User ID who created the evidence
* @param {string} caseNumber - Case number these evidence items belong to
* @returns {Promise<void>}
*/
...
try {
  ...
  await batch.commit();
  console.log(`Batch saved ${evidenceItems.length} evidence items`);
} catch (error) {
...
/**
* Link existing evidence to a new case
* @param {string} evidenceId - Evidence ID to link
* @param {string} caseNumber - Case number to link to
* @returns {Promise<void>}
*/
...
try {
  ...
  console.log(`Evidence ${evidenceId} linked to case ${caseNumber}`);
} catch (error) {
...
export const loadCaseWithAnnotations = async (caseId) => {
  try {
    ...
    const result = {
      ...
      // evidenceItems removed - evidence is loaded separately via loadEvidenceByCase
      technicalTerms: caseData.technicalTerms || [],
    };
