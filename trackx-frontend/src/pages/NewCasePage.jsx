...
import { 
  Upload, Info, CheckCircle, AlertCircle, FileText, Home, FilePlus2, 
  FolderOpen, Briefcase, LayoutDashboard, UserPlus, X, Shield, AlertTriangle,
  Search, Link2  // ← ADD THESE TWO ICONS
} from "lucide-react";
...
// Firebase services - ADD EVIDENCE FUNCTIONS
import {  
  updateCaseAnnotations,  
  getCurrentUserId,
  loadAllEvidence,      // ← ADD
  searchEvidence,       // ← ADD
  linkEvidenceToCase,   // ← ADD
  batchSaveEvidence     // ← ADD
...
function NewCasePage() {
  ...
  const linkExistingEvidence = (evidence) => {
    ...
    setEvidenceItems(prev => [...prev, {
      ...evidence,
      caseNumber: caseNumber || evidence.caseNumber, // Use current case number
    }]);
    ...
    console.log(`Linked evidence ${evidence.id} to case ${caseNumber}`);
  };
  ...
  const handleCreateCase = async () => {
    try {
      ...
      const payload = {
        ...
        // evidence_items removed - evidence is saved separately in evidence collection
      ...
      // ✅ 3.5) Save evidence items to dedicated evidence collection
      if (evidenceItems && evidenceItems.length > 0) {
        try {
          console.log(`Saving ${evidenceItems.length} evidence items to Firebase...`);
          ...
          // Don't fail the whole case creation if evidence save fails
          // The evidence is still saved in the case mirror, just not in the dedicated collection
          openModal({
            variant: "warning",
            title: "Evidence Warning",
            description: "Case created successfully, but there was an issue saving evidence items to the evidence database. Evidence is still attached to your case.",
          });
        }
      } else {
